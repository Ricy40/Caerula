...
public class SeacowAi {
    ...
    private static void initIdleActivity(Brain<Seacow> brain) {
        brain.addActivity(Activity.IDLE, 10, ImmutableList.of(
                ...
                new AnimalMakeLove(ModEntityTypes.SEACOW.get(), SPEED_MULTIPLIER_WHEN_MAKING_LOVE),
                new RunOne<>(ImmutableList.of(
                        Pair.of(new FollowTemptation(SeacowAi::getSpeedModifier), 1),
                        Pair.of(new BabyFollowAdult<>(ADULT_FOLLOW_RANGE, SPEED_MULTIPLIER_WHEN_FOLLOWING_ADULT_IN_WATER), 1),
    ...
    public static void setFoodLocation(Seacow seacow, BlockPos pos) {
        if (seacow.level.getWorldBorder().isWithinBounds(pos) && seacow.getBrain().getMemory(ModMemoryModuleTypes.FOOD_POS.get()).isEmpty()) {
