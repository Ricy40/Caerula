execute if entity @p[nbt={Inventory:[{id:"minecraft:coal"}]}] if score @s fuel_level < @s fuel_max run scoreboard players add @s fuel_level 1
execute unless entity @p[nbt={Inventory:[{id:"minecraft:coal"}]}] if score @s fuel_level < @s fuel_max run title @p title {"text":"Gather more coal","color":"red","bold":true}

execute if score @s fuel_level < @s fuel_max run clear @p minecraft:coal 1
...
scoreboard players enable @p buggy_optns